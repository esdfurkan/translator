...
def start_cli(lang, profile):
    print("--- Sade ve Akıllı Çeviri Aracı ---")
    ...
    if saved_api_key:
        ...
        print(lang.get('INFO_EXAMPLE_KEY', '...'))
    ...
    if profile_exists:
        ...
        print(lang.get('INFO_PROFILE_LANG', '> Interface Language: {lang}').format(lang=lang_code))
        print(lang.get('INFO_PROFILE_TARGET', '> Target Language: {target}').format(target=target_language))
        print(lang.get('INFO_PROFILE_MODEL', '> Model: {model}').format(model=selected_model))
        
        models_path = "models.env"
        if not os.path.exists(models_path):
            print(f"HATA: '{models_path}' dosyası bulunamadı."); return
        models = {}
